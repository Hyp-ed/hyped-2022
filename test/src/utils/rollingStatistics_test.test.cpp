   ...
   namespace utils {
     ...
     */
     struct rollingStatistics_test_int : public ::testing::Test 
     {
       ...
       hyped::utils::math::RollingStatistics<int> test_stats_int = hyped::utils::math::RollingStatistics<int>(window);
     ...
     */
     struct rollingStatistics_test_float : public ::testing::Test 
     {
       protected:
       std::size_t window = 2000;
       hyped::utils::math::RollingStatistics<float> test_stats_float = hyped::utils::math::RollingStatistics<float>(window);
     ...
     */
     TEST_F(rollingStatistics_test_int, test_var_std_int_rolling)
     {
       ...
       {
         test_stats_int.update(values[i]);
         ASSERT_EQ((int) sqrt(test_stats_int.getVariance()) , test_stats_int.getStdDev());
       }
     }
     ...
     */
     TEST_F(rollingStatistics_test_float, test_var_std_float_rolling)
     {
       ...
       {
         test_stats_float.update(values_f[i]);
         ASSERT_EQ((float) sqrt(test_stats_float.getVariance()) , test_stats_float.getStdDev());
       }
     }
     ...
     */
     ...
     TEST_F(rollingStatistics_test_int, test_int_outliers_rolling)
     {
       ...
       for (int i=0;i < ((int) values_counter/10);i++)
       {
     ...
     */
     TEST_F(rollingStatistics_test_float, test_float_window_rolling)
     {
       ...
       for (int i=0; i <= ((int) values_counter/10);i++) 
       {
         float new_variable = RandomFloat(2000,3000); //we will purposely assign a larger value for easy comparison
...
/**
 * Check if statistics update correctly if window size is exceeded (new elments being integer outliers if considering statistics 
 * for the original set) and last elements are removed from calculations.
 * 
 * */
...
TEST_F(rollingStatistics_test_int, test_int_window_rolling)
{
  ...
  for (int i=0; i <= ((int) values_counter/10);i++) {
  int new_variable = rand() % 3000 + 2000; //we will purposely assign a larger value for easy comparison
