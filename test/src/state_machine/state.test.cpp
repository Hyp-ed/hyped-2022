/*
 * Author: Efe Ozbatur, Franz Miltz
 * Organisation: HYPED
 * Date: 06.04.2021
 *
...
TEST_F(IdleTest, handlesCalibrateCommand)
{
  ...
  for (int i = 0; i < TEST_SIZE; i++) {
    ...
    if (!has_emergency) {
      ...
      if (!calibrate_command) { ASSERT_EQ(new_state, nullptr); }
    }
  }
}
...
TEST_F(NominalBrakingTest, handlesStopped)
{
  ...
  for (int i = 0; i < TEST_SIZE; i++) {
    ...
    if (!has_emergency) {
      ...
      if (stopped) { ASSERT_EQ(new_state, Finished::getInstance()); }
    }
  }
}
...
TEST_F(FailureBrakingTest, handlesStopped)
{
  ...
  for (int i = 0; i < TEST_SIZE; i++) {
    ...
    if (stopped) { ASSERT_EQ(new_state, FailureStopped::getInstance()); }
  }
}
