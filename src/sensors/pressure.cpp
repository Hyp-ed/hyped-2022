...
Pressure::Pressure(hyped::utils::Logger &log, uint16_t pressure_pin, uint16_t temp_pin)
...
uint16_t Pressure::scaleData(uint16_t raw_temp_value, uint16_t raw_pressure_value)
{
  ...
  uint16_t pressure = static_cast<int>((P1 + P2 - P3 - P4) / 100);

  return pressure;
}

uint16_t Pressure::getData()
{
